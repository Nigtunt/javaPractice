package Algorithm.dataStructure.BTree;
...
public class BTree2<K, V>
{
    ...
    //
    private int t = DEFAULT_DEGREE;
    /** 非根节点中最小的键值数 */
    private int minKeySize = (int)(Math.ceil(t/2.0)-1);
    /** 非根节点中最大的键值数 */
    private int maxKeySize = t;
    /** 键的比较函数对象 */
    ...
    public BTree2(int t)
    {
        this();
        this.t = t;
        minKeySize = (int)(Math.ceil(t/2.0)-1);
        maxKeySize = t;
    }
...
//            children = newChildren;
    ...
    private void splitNode(BTreeNode<K, V> parentNode, BTreeNode<K, V> childNode, int index)
    {
        ...
        // temp 为节点中 中间之后的一个元素
        int temp = (maxKeySize%2==0)? (maxKeySize / 2):(maxKeySize / 2 + 1);
        // 将中间之后的元素放到新节点中
        for(int i = 0; i < minKeySize; ++ i)
            siblingNode.addEntry(childNode.entryAt(temp + i));
        // 提取满子节点中的中间项，其索引为(temp - 1)
        Entry<K, V> entry = childNode.entryAt(temp - 1);
        // 删除满子节点中索引为[t - 1, 2t - 2]的t个项
        for(int i = maxKeySize - 1; i >= temp - 1; -- i)
            childNode.removeEntry(i);
        if(!childNode.isLeaf()) // 如果满子节点不是叶节点，则还需要处理其子节点
        {
            // 将满子节点中索引为[temp, 2temp - 1]的t个子节点插入新的节点中
            for(int i = 0; i < minKeySize + 1; ++ i)
                siblingNode.addChild(childNode.childAt(temp + i));
            // 删除满子节点中索引为[temp, 2temp - 1]的t个子节点
            for(int i = maxKeySize; i >= temp; -- i)
    ...
    {
        // 该过程需要保证，对非根节点执行删除操作时，其关键字个数至少为minKeySize。
        assert node.size() >= minKeySize || node == root;
        ...
        if(result.isExist())
        {
            ...
            {
                // 2.a 如果节点node中前于key的子节点包含至少minKeySize个项
                BTreeNode<K, V> leftChildNode = node.childAt(result.getIndex());
                if(leftChildNode.size() >= minKeySize)
                {
                ...
                else
                {
                    ...
                    if(rightChildNode.size() >= minKeySize)
                    {
        ...
        else
        {
            ...
            BTreeNode<K, V> childNode = node.childAt(result.getIndex());
            if(childNode.size() >= minKeySize) // // 如果子节点有不少于t个项，则递归删除
            ...
            else // 3
            {
                ...
                {
                    if(node.childAt(result.getIndex() + 1).size() >= minKeySize)
                    {
    ...
    public void output()
    {
        ...
        HashMap<BTreeNode<K, V>,Integer> map = new HashMap<>();
        queue.offer(root);
        map.put(root,0);
        int cur = -1;
        while(!queue.isEmpty())
        {
            ...
            {
                for(int i = 0; i <= node.size(); ++ i){
                    BTreeNode<K, V> kvbTreeNode = node.childAt(i);
                    queue.offer(kvbTreeNode);
                    map.put(node.childAt(i),map.get(node) + 1);
                }
    ...
    public static void main(String[] args)
    {
        ...
        BTree2<Integer, Integer> btree = new BTree2<Integer, Integer>(6);
